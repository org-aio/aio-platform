...
nuxt 全栈插件，包含前端计数、服务端计数接口和共享 TypeScript 模型。
...
后端使用 Nuxt Nitro server routes；`POST /api/counter` 接收 `{"value": 41}`，返回计数值 `42` 和宿主注入的租户。
...
框架说明：[Nuxt Node 服务部署](https://nuxt.com/docs/4.x/getting-started/deployment)。
