async function warmFrontendAssets(config, signal, prepare = async () => {}) {
  ...
  const activePageIds = () => new Set(
    [...document.querySelectorAll('[data-aio-page-active="true"]')].map(page => page.dataset.aioPage).filter(Boolean)
  );
  const seen = new Set();
  ...
  for (const page of pages) {
    ...
    // 当前页仍处于首屏关键路径，后台预热不得抢占它的资源请求。
    if (activePageIds().has(page.id)) continue;
