...
pub(super) async fn report(
    ...
    if updated.rows_affected() != 1 {
        return Err(RuntimeError::unauthorized("设备已撤销"));
    }
...
fn validate_inventory(inventory: &Inventory) -> Result<()> {
        ...
        inventory.tools.len() <= 1000 && inventory.packages.len() <= 1000,
        "安装记录过多"
    );
    for item in inventory.tools.values() {
        ...
        inventory.error.as_ref().is_none_or(|s| s.len() <= 1000),
        "检测错误信息过长"
    );
    Ok(())
}
...
async fn inventories(
...
) -> Result<BTreeMap<String, (Inventory, i64, bool)>> {
    ...
    rows.into_iter()
        .map(|(id, value, at, fresh)| Ok((id, (serde_json::from_value(value)?, at, fresh))))
        .collect()
}
...
fn installed(
    ...
    let record = inventory.tools.get(&manifest.id).cloned();
    if record
        .as_ref()
        .is_some_and(|r| !["installed", "executed"].contains(&r.state.as_str()))
    {
        return record;
    }
    ...
    let Some(command) = manifest
        .platforms
        .get(platform)
        .and_then(|plan| plan.install.first())
    else {
        return record;
    };
    if command.program != "npm"
        ...
        || command.args[1] != "--global"
    {
        return record;
    }
    let Some((package, _)) = command.args[2].rsplit_once('@') else {
        return record;
    };
    if let Some(version) = inventory.packages.get(package) {
...
async fn device_tools(
    state: &RuntimeState,
    session: &SessionContext,
    manifest: &ToolManifest,
) -> Result<Vec<DeviceTool>> {
    let inventories = inventories(state, session).await?;
    let workers = state.workers.list(session).await?;
    Ok(workers
        .into_iter()
        .filter(|w| ["online", "offline"].contains(&w.status.as_str()))
        .map(|device| {
            let snapshot = inventories.get(&device.id);
            let platform = match device.platform.as_str() {
                "darwin" => "macos",
                "win32" => "windows",
                other => other,
            };
            DeviceTool {
                installed: snapshot
                    .and_then(|(value, _, _)| installed(manifest, &device.platform, value)),
                checked_at: snapshot.map(|(_, at, _)| *at),
                fresh: snapshot.is_some_and(|(_, _, fresh)| *fresh) && device.status == "online",
                can_install: snapshot.is_some_and(|(value, _, fresh)| {
                    value.can_install && *fresh && value.error.is_none()
...
pub(super) async fn list(
    ...
    headers: HeaderMap,
    Path(id): Path<String>,
) -> Result<Json<RuntimeResponse<Vec<DeviceTool>>>, RuntimeError> {
    let session = authenticate(&state, &headers).await?;
    let manifest = storage::entries(&state.store.pool)
        .await?
        .into_iter()
        .find_map(|entry| match entry {
            MarketplaceItem::Cli(entry) if entry.cli.id == id => Some(entry.cli),
            _ => None,
        })
        .ok_or_else(|| RuntimeError::not_found("CLI 条目不存在"))?;
    Ok(Json(RuntimeResponse {
        data: device_tools(&state, &session, &manifest).await?,
    }))
}
...
pub(super) async fn install(
    ...
    let session = authenticate(&state, &headers).await?;
    let manifest = storage::get(&state.store.pool, &id, &request.version)
        .await?
        .ok_or_else(|| RuntimeError::not_found("CLI 条目不存在或已删除"))?;
    let devices = device_tools(&state, &session, &manifest).await?;
    let target = devices
        .iter()
        .find(|d| d.device.id == request.worker_id)
        .ok_or_else(|| RuntimeError::not_found("设备不存在"))?;
    if !target.can_install || !target.supported {
        return Err(RuntimeError::bad_request(
            "设备离线、检测失败或本机助手需要升级",
        ));
    }
    if target.installed.is_some() {
        return Err(RuntimeError::bad_request(
            "设备已有安装记录，请先在设备上处理现有版本",
        ));
    }
    let task = state
        .workers
...
mod tests {
    ...
    #[test]
    fn npm_changes_override_old_success_records_but_preserve_failed_installations() -> Result<()> {
        ...
        inventory
            .packages
            .insert("codex-model-sync".into(), "0.4.2".into());
        assert_eq!(
            installed(&manifest, "darwin", &inventory).map(|r| r.version),
            Some("0.4.2".into())
        );
        inventory.packages.clear();
        assert_eq!(
            installed(&manifest, "darwin", &inventory).map(|r| r.state),
            Some("missing".into())
        );
        inventory.tools.insert(
            manifest.id.clone(),
