function mountFrontendAssets(frame, config, active, currentTicket = () => config.token) {
  ...
  const receive = async event => {
    ...
    try {
      ...
      if (!active() || !ticket || controller.signal.aborted) throw new Error('插件资源请求已暂停');
      const result = await load(message.path, ticket, controller.signal);
      if (!active() || await currentTicket() !== ticket || controller.signal.aborted) throw new Error('插件资源请求已暂停');
