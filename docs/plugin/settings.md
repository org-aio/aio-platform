...
v2 插件在清单声明自己的设置页，宿主在“账户菜单 → 设置中心 → 插件设置”及已安装插件的市场详情中提供统一入口。业务页面统一放入工作空间，设置页不进入业务菜单或账户菜单。
...
组件 describe 或进程 /aio/describe 同时声明 id=settings 的页面，surface 使用已有 fullscreen，entry 指向 settings.html，scene 为空、menu_path 为空。宿主校验页面确实存在并按原有权限过滤，不需要升级 WIT enum。页面在独立沙箱 iframe 中打开，通过相同 AIO Web SDK 调用插件自身接口，租户/用户身份仍由宿主注入。
