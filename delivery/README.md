...
网络需要代理时，`AIO_BUILD_HTTP_PROXY` 可指向构建容器能访问的无凭据 HTTP 代理，统一应用于源码获取和支持 HTTP 代理的构建工具。代理地址不得包含用户名或密码；发布与数据库凭据仍只留在宿主。252 使用[独立网络出口](egress/README.md)，无需开发机在线；已有活动版本不受构建网络中断影响。
