...
测试覆盖桌面/移动端真实壳、计数及前后端调用，并保存 Playwright HTML/JSON 报告、截图和失败追踪到 `target/development-report/`。需要本机 Chrome。可通过 `AIO_SANDBOX_REPORT_DIR` 指定独立报告目录，保留不同环境的验收结果。不要提交含数据库连接或会话凭据的运行目录。
...
`AIO_TEST_CLI=/path/to/installed/aio node tests/development/distribution.cjs` 使用实际安装入口创建三种新插件，并分别记录新项目构建和缓存重启耗时。此用例清除宿主与 Web 路径覆盖，确保只使用分发包携带的宿主；所有进程在退出时停止，项目数据保留在 `target/distribution-acceptance/`。
